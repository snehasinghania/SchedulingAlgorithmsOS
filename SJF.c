...
int* checksjf(int time , struct Process* process, int num_pro)  // checks if a process has arrived at the given time
{
	int li[num_pro];
	li[0] = 0;
	int i = 1 ;
...
int min_service(struct Process* process, int* li , int count) // returns the process with minimum service time among the arrived processes
{
	...
	for(i=1 ; i <= count ; i++)
	{
		...
		if(li[i] != 0)
		{
			if(process[i].service_time < min)
			{
				min_st_pro = i;
				min = process[i].service_time;
			}
		}
	}
	...
	for(i = 1 ; i <= count ; i++)
	{
		if(li[i] != 0)
		{
			if(process[i].service_time == min && process[i].arrival_time < min_arr)
			{
				min_st_pro = i;
				//printf("hello %c\n" ,process[i].pro_id);
			}
...
void SJF(struct Process* process , int count)
{
	printf("SFJ\n");
	...
	{
		int *liss;
		liss  = checksjf(time, process, count);
		li = min_service(process,liss,count);
